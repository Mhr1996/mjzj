<template>
    <div class="amamasmded" v-if="loading">
        ...
        <div class="cgf mlr_12 oh br">
            ...
            <div class="csb plr_20 lh50 bbeb rj">
                <span>订单号码</span> <span class="c9">{{rd.order_no}}</span>
            </div>
            <div class="csb plr_20 lh50 rj" v-if="type!=1">
                <span>物流状态</span> <span class="c9">{{rd.state==1?'待付款':rd.state==2?'待发货':rd.state==3?'待收货':rd.state==4?'已完成':rd.state==5?'退换货订单':'用户取消'}}</span>
            </div>
        </div>
        ...
        methods: {
            ...
            getOrderDetail(id) {
                ...
                this.$http.post('/wxsite/goods/api', { 
                    api_name:  this.type==1 ? 'RecommendOrderDetail' :  'handleRecommendDetail',
                    id
                }).then(res => {
                    this.loading = true;
                    this.$toast.clear();
